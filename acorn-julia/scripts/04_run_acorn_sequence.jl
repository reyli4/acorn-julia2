...
elseif haskey(config, "sim_year_sequence")
    ...
    sim_sequence = collect(to_int_year(sim_years[1]):to_int_year(sim_years[2]))
end
...
open(manifest_path, "w") do io
    write(io, "seq_idx,sim_year,token,prev_token\n")
    ...
    for (i, sim_year) in enumerate(sim_sequence)
        ...
        println("Now running sequence step $(i)/$(length(sim_sequence)): year $(sim_year), token=$(token)")
        flush(stdout)
        ...
        if isfile("$(out_dir)/load_shedding_$(token).csv")
            ...
            write(io, "$(i),$(sim_year),$(token),$(prev_token)\n")
            continue
        end
